...
class PostTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.coord_group = Group.objects.create(name='coordinator')
        self.coord_group.user_set.add(self.coord_user)
    ...
    def test_nonowner_get_threads(self):
    ...
    def test_nonowner_get_archived_threads(self):
        self.api_client.force_authenticate(user=self.coord_user)
    ...
    def test_owner_get_threads(self):
        self.api_client.force_authenticate(user=self.coord_user)

        cp = CaseParticipant.objects.create(case=self.case, contact=self.coord_user.contact, role="owner")
    ...
    def test_owner_get_archived_threads(self):
        self.api_client.force_authenticate(user=self.coord_user)
    ...
    def test_multi_thread_creation(self):
        ...
        CaseParticipant.objects.create(case=self.case, contact=self.coord_user.contact, role="owner")
        CaseParticipant.objects.create(case=self.case, group=self.coord_group, role="owner")
        ...
        response = self.api_client.post(
            reverse('cvdp:threadlistapi', args=['111111']),
            data=json.dumps({'subject': 'thread title #2'}),
            content_type='application/json')
                         
        
        #forbidden since coordinator is not owner and they just created one
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_delete_post(self):
        ...
        posts = Post.objects.filter(thread=thread).first()

        response = self.api_client.delete(
            reverse('cvdp:postapi', args=[posts.id]))

        self.assertEqual(response.status_code, status.HTTP_202_ACCEPTED)

    def test_vendor_delete_post(self):
        self.api_client.force_authenticate(user=self.coord_user)
        contact = Contact.objects.get(user=self.coord_user)
        CaseParticipant.objects.create(case=self.case, contact=contact, role="owner")
        thread = CaseThread.objects.filter(case=self.case, archived=False).first()
...
class CaseParticipantTest(APITestCase):
    ...
    def test_coordinator_get_participants(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        cps = CaseParticipant.objects.filter(case=self.case).order_by('title')
        serializer = CaseParticipantDetailSerializer(cps, many=True)
    ...
    def test_vendor_get_participants(self):
        ...
        #fields parameter are fields that are ignored from this serializer
