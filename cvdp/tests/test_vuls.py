...
class VulTests(APITestCase):
    ...
    def test_coordinator_get_vuls(self):
        ...
        serializer = VulSerializer(vuls, many=True, context={'user': self.coord_user})
    ...
    def test_coordinator_get_vul(self):
        ...
        serializer = VulSerializer(vuls, context={'user': self.coord_user})
        self.assertEqual(response.data, serializer.data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_coordinator_update_invalid_references2(self):
        self.api_client.force_authenticate(user=self.coord_user)

        vul = Vulnerability.objects.filter(case=self.case, deleted=False).first()

        # must be an object with url (and optionally "summary")
        
        valid_payload = {
            'references': ["https:www.thisisnotvalid.because.its.not.an.array"]
        }
    ...
    def test_coordinator_update_valid_references(self):
        ...
        valid_payload = {
            'references': [{"url": "https:www.thisisvalid.com"}, {"url": "www.badguys.net", "summary": "reference"}]
        }
    ...
    def test_coordinator_score_cvss(self):
        ...
        cvss = VulCVSS.objects.filter(vul=vul)
        serializer = CVSSSerializer(cvss, many=True)
        self.assertEqual(response.data, serializer.data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        valid_payload = {
            ...
            'RL': 'X',
            'RC': 'X',
            'vectorString': 'CVSS:3.1/AV:A/AC:L/PR:H/UI:R/S:U/C:N/I:L/A:H',
            'score': '4.9',
            'version': '3.1',
            'severity': 'Medium'}
        ...
        #test remove
        response = self.api_client.delete(
            reverse('cvdp:cvss_version_api', args=[vul.id, '3.1']))

        self.assertEqual(response.status_code, status.HTTP_202_ACCEPTED)


    def test_coordinator_remove_cvss(self):
        self.api_client.force_authenticate(user=self.coord_user)
    ...
    def test_coordinator_score_cvss_invalid(self):
        ...
        invalid_payload = {
            ...
            'score': '5.4',
            'severity': 'Medium'}
        #need vectorString above since it is required 
        #'vectorString': 'CVSS:3.1/AV:N/AC:H/PR:L/UI:R/S:U/C:N/I:L/A:H',
...
class VulStatusTests(APITestCase):
    ...
    def setUp(self):
        ...
        GroupProfile.objects.create(group=self.vendor_group)
        self.vendor_group.user_set.add(self.vendor_user)

        self.case = Case.objects.create(case_id = '111111', title='Test Case', summary="This is a summary of test case", status=Case.ACTIVE_STATUS)
        cp = CaseParticipant.objects.create(case=self.case, group=self.vendor_group, notified=timezone.now())
        contact = Contact.objects.get(user=self.coord_user)
        CaseParticipant.objects.create(case=self.case, contact=contact, role="owner")
        Vulnerability.objects.create(case=self.case, description="Let's do this!")
    ...
    def test_coordinator_add_component_status(self):
        ...
        valid_payload = {
            ...
            'owner': str(self.vendor_group.groupprofile.uuid),
        }
        ...
        #component must have owner or supplier
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

        valid_payload['component']['supplier'] = 'vendor'
        ...
        serializer = StatusSummarySerializer(cs, many=True, context={'user': self.coord_user})
        ...
        serializer = StatusSummarySerializer(comp, context={'user': self.coord_user})
        ...
        valid_payload = {
            ...
            'vuls':[vul.id],
            'statement': "WE affected",
            'component': {'name': 'django', 'supplier': 'vendor'}
        }
        response = self.api_client.patch(
            reverse('cvdp:statusapi-detail', args=[comp.id]),
            data=json.dumps(valid_payload),
    ...
    def test_vendor_add_component_status(self):
        ...
        valid_payload = {
            ...
            'statement': "WE affected",
            'component': {'name': 'django', 'owner': str(self.vendor_group.groupprofile.uuid)},
        }
        response = self.api_client.post(
            reverse('cvdp:statusapi', args=['111111']),
            data=json.dumps(valid_payload),
        ...
        cs = ComponentStatus.objects.filter(vul__case=self.case)
        serializer = StatusSummarySerializer(cs, many=True, context={'user': self.vendor_user})
        ...
        serializer = StatusSummarySerializer(comp, context={'user': self.vendor_user})
    ...
    def test_vendor_other_component_status(self):
        ...
        prod = Product.objects.create(component=c)
        
        cs = ComponentStatus.objects.create(vul=vul, component=c)
        cs.add_revision(StatusRevision(version_status=[{'version_value':"1.2.3", 'status':'Affected'}], statement='Vendor should not see this'))
