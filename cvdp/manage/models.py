...
from cvdp.forms import GenReportingForm, GenSubForm
...
class AbstractReportingForm(models.Model):
    ...
    """
    def get_sub_form(self, *args, **kwargs):
        ...
        logger.debug(fields)
        return GenSubForm(*args, extra=fields, **kwargs)
    

    def has_subforms(self):

        for question in self.fields.all():
            if question.question_type == 12:
                return True
    ...
    def get_subforms(self):
        ...
        for question in self.fields.all():
            if question.question_type == 12:
                #lookup question as form title
                subform = ReportingForm.objects.filter(title=question.question).first()
                if subform:
                    formsets.append(subform)
        return formsets
    """
        ...
        for i, (k, v) in enumerate(data.items()):
            if fields[i].question_type in [3, 5, 8]:
                ...
                if v:
                    ...
                    answers.append({'question': fields[i].question, 'answer': '', 'priv': fields[i].private})
            elif fields[i].question_type == 7 and isinstance(v, (datetime, date)):
                answers.append({'question': fields[i].question, 'answer': v.isoformat(), 'priv': fields[i].private})
            else:
...
class AbstractFormEntry(models.Model):
    ...
    report_id = models.CharField(
        max_length=50,
        help_text=_("Optional, but helpful for preventing duplicate entries"),
        blank=True,
        null=True)
...
class FormEntry(AbstractFormEntry):
    ...
    title = models.CharField(
        help_text=_("Optional, helpful for custom reports"),
        max_length=300,
        blank=True,
        null=True)
...
class AbstractFormQuestion(models.Model):
    ...
    max_iterations = models.IntegerField(
        default = 0,
        help_text=("Max number of iterations for this question group"),
    )
    """
...
class AdVISEConnection(models.Model):
    ...
    class Meta:
        verbose_name_plural = "VINCE-NT Connections"
        verbose_name = "VINCE-NT Connection"
...
class AdviseTask(models.Model):
    ...
    title=models.CharField(
        _("A brief summary of the task performed for audit purposes"),
        blank=True,
        null=True,
        max_length=500)
    
    class Meta:
        verbose_name_plural = "VINCE-NT Tasks"
        verbose_name = "VINCE-NT Task"
...
class AdviseScheduledTask(models.Model):
    ...
    timeout = models.IntegerField(
        help_text=_('Number of seconds after which a job in "running" state will be considered dead and will be allowed to run again. Use 0 to disable. NOTE: This will not actually kill the job if it is still running!'),
        default=0,
        validators=[
            MinValueValidator(0, message="Only zero and positive values are valid.")
        ]
    )
    ...
    class Meta:
        verbose_name_plural = "VINCE-NT Scheduled Tasks"
        verbose_name = "VINCE-NT Scheduled Task"
...
class VulAttributeKey(models.Model):
    ...
    user = models.ForeignKey(
        ...
        blank=True,
        null=True,
	help_text=_('User that added this attribute.'),
        verbose_name=_('User'),
    )
    ...
    description = models.CharField(
        max_length=500,
        help_text=_('Description of attribute'),
        blank=True,
        null=True
    )
...
class CSAFProfile(models.Model):
	...
	Group,
        on_delete=models.SET_NULL,
        help_text=_('Coordination Team'),
        blank=True,
        null=True
    )
    
    publisher_settings = models.JSONField(
        help_text=('Publisher CSAF Settings'),
        default = dict
    )

    notes = models.JSONField(
        help_text=('Document Notes'),
        default = list
    )

    references = models.JSONField(
        help_text=('Document References'),
        default = list
    )
	...
	on_delete=models.CASCADE,
        blank=True,
	null=True,
        help_text=_('User that created this profile.'),
	verbose_name=_('User'),
    )
    ...
    def __str__(self):
        return f"CSAF Profile: {self.name}"
