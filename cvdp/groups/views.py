...
from cvdp.lib import send_template_email, create_contact_action, create_group_action, create_contact_change, create_api_account, notify_group_admin
...
def _unverify_contact(instance):
    if instance.contact.user:
        instance.group.user_set.remove(instance.contact.user)
        logger.debug(f"Removed {instance.contact.user} from {instance.group}")
...
class ContactActivityAPIView(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.kwargs.get('contact'):
            ...
            if not(is_coordinator(self.request.user) or self.has_contact_permission(contact, self.request.user)):
                raise PermissionDenied
            actions = ContactAction.objects.filter(contact=contact)
            if not is_coordinator(self.request.user):
                actions = actions.exclude(share=False)
...
class GroupAdminView(LoginRequiredMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        my_groups = self.request.user.groups.exclude(groupprofile__isnull=True)

        #get all groups this user is the admin for

        context['admin_groups'] = list(ContactAssociation.objects.filter(group__in=my_groups, contact__user=self.request.user, group_admin=True).values_list('group__groupprofile__uuid', flat=True))
...
class GroupSearchView(LoginRequiredMixin, UserPassesTestMixin, generic.ListView):
        ...
        if self.request.user.is_authenticated:
            if not is_coordinator(self.request.user):
                if self.kwargs.get("group"):
                    group = Group.objects.filter(groupprofile__uuid=self.kwargs['group']).first()
                    if group:
    ...
    def post(self, request, *args, **kwargs):
        logger.debug(f"Group search view : {self.request.POST}")
        search = self.request.POST.get('search')
        thread = self.request.POST.get('thread')
        case = self.request.POST.get('case')
        tags = DefinedTag.objects.filter(tag__icontains=search, category=3).values_list('tag', flat=True)
        groupTag = []
        tags_populated = []
        if (tags and search):
            groupTag = GroupTag.objects.filter(tag__in=tags).distinct('tag')
            tags_populated = groupTag.values_list('tag', flat=True)
            groupTag = groupTag.values_list('group__id', flat=True)
        
        groups = Group.objects.filter(Q(name__icontains=search) | Q(id__in=groupTag)).exclude(groupprofile__active=False).exclude(groupprofile__isnull=True)
        ...
        for g in tags_populated:
            results.append({'name': g, 'photo': None, 'logocolor': None, 'uuid': g, 'tag': True} )
        for g in groups[:10]:
        ...
        for c in contacts[:10]:
            associations = list(c.associations.filter(verified=True).values_list('group__name', flat=True))
            results.append({'name':c.get_name(), 'email': c.email, 'logocolor':c.get_color(), 'photo': c.get_photo(), 'uuid': c.uuid, 'type': 'contact', 'groups': associations})
...
class GroupAPIFilter(django_filters.FilterSet):
    ...
    type = django_filters.MultipleChoiceFilter(
        field_name='type',
        method="filter_type",
        choices=[('1', 'group'), ('2', 'contact'), ('3', 'user'), ('4', 'tag')],
        label='type')
...
class GroupAPIView(APIView):
    ...
    def get(self, request, format=None):
        ...
        if search_type not in ['group', 'groups', 'all', 'contacts', 'users', 'tag']:
            return Response({'type': 'invalid type'}, status=status.HTTP_400_BAD_REQUEST)


        if not is_coordinator(self.request.user):
            #ONLY GET MY GROUPS
            my_groups = self.request.user.groups.exclude(groupprofile__isnull=True).values_list('groupprofile__uuid', flat=True)
            if search_term:
                groups = GroupProfile.objects.filter(uuid__in=my_groups, group__name__icontains=search_term).order_by('-modified')
            else:
                groups = GroupProfile.objects.filter(uuid__in=my_groups).order_by('-modified')
        else:

            if (search_term):

                if search_type == "tag":
                    tags = DefinedTag.objects.filter(tag__icontains=search_term, category=3).values_list('tag', flat=True)
                    groupTag = []
                    if (tags):
                        groupTag = GroupTag.objects.filter(tag__in=tags).values_list('group__id', flat=True)
                    groups = GroupProfile.objects.filter(group__id__in=groupTag).order_by('-modified')
                if search_type in ['group', 'groups', 'all']:
                    tags = DefinedTag.objects.filter(tag__icontains=search_term, category=3).values_list('tag', flat=True)
                    logger.debug(tags)
                    groupTag = []
                    if (tags):
                        groupTag = GroupTag.objects.filter(tag__in=tags).values_list('group__id', flat=True)
                        logger.debug(groupTag)
                    groups = GroupProfile.objects.filter(Q(group__name__icontains=search_term) | Q(group__id__in=groupTag)).order_by('-modified')
                if search_type in ['contacts', 'all', 'users']:
                    if search_type == "users":
                        contacts = Contact.objects.filter(Q(name__icontains=search_term)|Q(email__icontains=search_term)|Q(user__screen_name__icontains=search_term)).exclude(user__api_account=True).exclude(user__screen_name__isnull=True).order_by('-modified')
                    else:
                ...
                if search_type in ['contacts', 'all', 'users']:
                    if search_type == "users":
                        contacts = Contact.objects.all().exclude(user__api_account=True).exclude(user__screen_name__isnull=True).order_by('-modified')
                    else:
                        contacts = Contact.objects.all().exclude(user__api_account=True).order_by('-modified')
        results = chain(groups, contacts)
        qs = sorted(results,
        ...
        serializer = GenericContactSerializer(gp.page(page_number), many=True, context=context)

        return Response({"results": serializer.data, "count": gp.count, "pages": gp.num_pages, "page_size": page_size}, status=status.HTTP_200_OK)
...
class GroupAdminAPIView(viewsets.ModelViewSet):
    ...
    def get_object(self):
        g = get_object_or_404(Group, groupprofile__uuid=self.kwargs['group'])
        self.check_object_permissions(self.request, g)
        return g

    def get_serializer_class(self, *args, **kwargs):
        if is_coordinator(self.request.user):
            return CoordGroupSerializer
        return self.serializer_class

    def get_queryset(self):
        if getattr(self, 'swagger_fake_view', False):
            return Group.objects.none()
        return self.request.user.groups.exclude(groupprofile__isnull=True)
    ...
    def update(self, request, **kwargs):
        ...
        if data.get('logocolor'):
            ...
            if len(logocolor) != 7:
                #this needs to be '#+ 6 digits' - if not add a 0
                logocolor = f"{logocolor}0"

            action = create_group_action(f"generated new logo color for group {instance.name}", request.user, instance)
            create_contact_change(action, "logo color", instance.groupprofile.icon_color, logocolor)
            instance.groupprofile.icon_color = logocolor
        elif data.get('logo'):
            if data['logo'] == "reset":
        ...
        elif data.get('permissions'):
            perms = data['permissions']
            if perms == "True":
                action = create_group_action(f"changed permission scheme to select cases", request.user, instance)
            else:
                action = create_group_action(f"changed permission scheme to all cases", request.user, instance)
            instance.groupprofile.permissions = perms
        else:
...
class GroupCasePermAPIView(viewsets.ModelViewSet):
    serializer_class = GroupCaseAccessPermSerializer
    permission_classes = (IsAuthenticated, GroupAdminLevelPermission, PendingUserPermission)

    def get_view_name(self):
        return "Group Case Access Permissions"

    def get_serializer_class(self, *args, **kwargs):
        if self.request.method == 'PATCH':
            return CaseAccessSerializer
        return GroupCaseAccessPermSerializer
    
    def get_queryset(self):
        g = get_object_or_404(Group, groupprofile__uuid=self.kwargs['group'])
        self.check_object_permissions(self.request, g)
        return CaseParticipant.objects.filter(group=g).exclude(notified__isnull=True).exclude(case__status=Case.PENDING_STATUS)

    def update(self, request, **kwargs):
        logger.debug(f"{self.__class__.__name__} update: {request.data}")
        g = get_object_or_404(Group, groupprofile__uuid=self.kwargs['group'])
        self.check_object_permissions(self.request, g)
        sc = self.get_serializer_class()
        serializer = sc(data=request.data)
        if serializer.is_valid():
            user_uuid = serializer.validated_data['user']
            #validate user and make sure they are in the group
            c = Contact.objects.filter(uuid=user_uuid).first()
            if not c:
                return Response({'user': 'invalid user'}, status=status.HTTP_400_BAD_REQUEST)
            if not c.user.groups.filter(id=g.id).exists():
                #this user doesn't belong here!
                return Response({'user': 'invalid user'}, status=status.HTTP_400_BAD_REQUEST)
            
            cases_present = []
            for perm in serializer.validated_data['perms']:
                case = Case.objects.filter(case_id=perm['case']).first()
                if not case:
                    return Response({'case': 'invalid case'}, status=status.HTTP_400_BAD_REQUEST)
                #check to see if this group has access to this case
                cp = CaseParticipant.objects.filter(case=case, group=g, notified__isnull=False).exclude(case__status=Case.PENDING_STATUS).first()
                if not cp:
                    return Response({'case': 'invalid case'}, status=status.HTTP_400_BAD_REQUEST)
                cases_present.append(perm['case'])
                old_perms = cp.permissions
                old_perms[user_uuid] = perm['perms']
                cp.permissions=old_perms
                cp.save()

            #set all other cases to no access:
            other_cases = CaseParticipant.objects.filter(group=g).exclude(case__case_id__in=cases_present).exclude(notified__isnull=True).exclude(case__status=Case.PENDING_STATUS)
            for oc in other_cases:
                if oc.permissions.get(user_uuid):
                    logger.debug(f"Removing Access for User to Case {oc.case.case_id}")
                    p = oc.permissions
                    del p[user_uuid]
                    oc.permissions = p
                    oc.save()
...
class GroupAPIAccountView(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        emails = g.groupprofile.support_emails
        if not emails:
            return Response({'message': 'organization email required'}, status=status.HTTP_400_BAD_REQUEST)

        new_account_email = create_api_account(g)

        if not new_account_email:
            return Response({'message': 'Unable to create unique API account. Please contact an admin.'}, status = status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            return Response({'message': 'Unable to create new API account. Please contact an admin.'}, status = status.HTTP_400_BAD_REQUEST)
...
class GroupDetailAPIView(viewsets.ModelViewSet):
    ...
    def update(self, request, **kwargs):
        ...
        if ('tags' in request.data):
            ...
            existing_tags = GroupTag.objects.filter(group = group).values_list('tag', flat=True)
            if len(existing_tags) > 0:
                for tag in existing_tags:
                    if tag not in request.data['tags']:
                        action = create_group_action(f"removed tag {tag}", request.user, group, False)
                        rmtag = GroupTag.objects.filter(group=group, tag=tag).first()
                        rmtag.delete()
                        
            for tag in request.data['tags']:
                if DefinedTag.objects.filter(tag = tag, category=3).exists():
                
                    tag, created = GroupTag.objects.update_or_create(group=group, tag=tag,
                                                                     defaults={'user':self.request.user})

                    if created:
                        action = create_group_action(f"added tag {tag}", request.user, group, False)
                else:
                    return Response({'tag': f"Tag {tag} does not exist."}, status=status.HTTP_400_BAD_REQUEST)    

            return Response({}, status=status.HTTP_202_ACCEPTED)

        if request.data.get('vendor_type'):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            return Response({'new': reverse('cvdp:group', args=[group.uuid])}, status=status.HTTP_201_CREATED)
        logger.debug(serializer.errors)
        return Response(serializer.errors,
                        status=status.HTTP_400_BAD_REQUEST)
...
class ContactAPIView(viewsets.ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        create_contact_action(f"removed {contact.email}/{contact.user} contact", self.request.user, contact)
        contact.delete()
    ...
    def create(self, request, *args, **kwargs):
        logger.debug(f"{self.__class__.__name__} create: {request.data}")
        #make sure email doesn't already exist
        if not request.data.get('email'):
            return Response({'email': 'this field is required.'}, status=
                            status.HTTP_400_BAD_REQUEST)

        instance = Contact.objects.filter(email = request.data['email']).first()
        if instance:
        ...
        if serializer.is_valid():
            ...
            action = create_contact_action(f"added contact {request.data['email']}", request.user, instance)
            return JsonResponse({'new':reverse("cvdp:contact", args=[instance.uuid])}, status=200)
        #return Response(serializer.data, status=status.HTTP_202_ACCEPTED)
        else:
            logger.debug(serializer.errors)
            return Response(serializer.errors,
    ...
    def update(self, request, **kwargs):
        ...
        if not is_coordinator(self.request.user):
            if data.get('group'):
                group = get_object_or_404(Group, groupprofile__uuid=data['group'])
                self.check_object_permissions(self.request, group)
            else:
                raise PermissionDenied()
        old_email = ""
        ...
        if serializer.is_valid():
            ...
            if old_email:
                ...
                instance.save()
                if instance.user:
                    action = create_contact_action(f"modified user associated with contact due to email change", request.user, instance)
                else:
                    action = create_contact_action(f"removed user associated with contact due to email change", request.user, instance)
                #make sure contact association happens?

                association = ContactAssociation.objects.filter(contact=instance)
                for ca in association:
                    ca.verified = False
                    ca.save()
                    ca.group.user_set.remove(old_user)
...
class CoordTeamAPIView(viewsets.ModelViewSet):
    permission_classes = (IsAuthenticated, PendingUserPermission, CoordinatorPermission)
    serializer_class = CoordGroupSerializer
    
    def get_view_name(self):
        return "Coordination Teams"
    ...
    def get_queryset(self):
        ...
        return Group.objects.filter(groupprofile__vendor_type='Coordinator')
    
    def create(self, request, *args, **kwargs):
        if not request.user.is_superuser:
            #only superusers can add coord teams
            raise PermissionDenied()
        logger.warning(request.data)
        if ('group' not in request.data):
            return Response({'group': 'missing required field'}, status = status.HTTP_400_REQUEST)
        group = get_object_or_404(Group, groupprofile__uuid=request.data['group'])
        group.groupprofile.vendor_type = 'Coordinator'
        group.groupprofile.save()
        create_group_action(f"added {group.name} as coordination team", self.request.user, group)
        return Response({}, status = status.HTTP_202_ACCEPTED)
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        group.groupprofile.vendor_type = 'Vendor'
        group.groupprofile.save()
        create_group_action(f"removed {group.name} as coordination team", self.request.user, group)
        return Response({}, status = status.HTTP_202_ACCEPTED)
    

class ContactAssociationRequestAPIView(viewsets.ModelViewSet):
    permission_classes = (IsAuthenticated, PendingUserPermission)
    serializer_class = ContactAssociationRequestSerializer

    def create(self, request, *args, **kwargs):
        logger.debug(request.data)
        serializer = self.serializer_class(data=request.data)
        contact = Contact.objects.filter(email = self.request.user.email).first()
        #let's only allow a max of 5 requests for this person
        prev_assoc = ContactAssociation.objects.filter(contact=contact, verified=False).count()
        if prev_assoc > 5:
            return Response({'error': "You have exceeded the maximum number of requests."}, status=status.HTTP_400_BAD_REQUEST)

        if serializer.is_valid():
            group = get_object_or_404(Group, groupprofile__uuid=request.data['group'])          
            ca, created = ContactAssociation.objects.update_or_create(contact=contact, group=group,
                                                                      defaults={'added_by': self.request.user})
            action = create_contact_action(f"user {contact.email} requested to join group {group.name}", request.user, contact)
            action.group = group
            action.save()

            notify_group_admin(group, contact)
...
class UnverifiedContactsAPIView(viewsets.ModelViewSet):
    permission_classes = (IsAuthenticated, PendingUserPermission, CoordinatorPermission)
    ...
    def get_queryset(self):
        ...
        return ContactAssociation.objects.filter(verified=False)
...
class ContactAssociationAPIView(viewsets.ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        log_action = "removed"
        if ca.verified == False:
            # create a rejected log - so users can't continually request access
            ra = RejectedAssociation.objects.update_or_create(contact=ca.contact, group=ca.group,
                                                              defaults={'rejected_by': self.request.user})
            log_action="rejected"
        
        if ca.contact.user:
            logger.debug("DELETING JUST THE ASSOC")
            action = create_contact_action(f"{log_action} user {ca.contact.email} from group {ca.group.name}", request.user, ca.contact)
            ...
            if other:
                action = create_contact_action(f"{log_action} email {ca.contact.email} from group {ca.group.name}", request.user, ca.contact)
                action.group=ca.group
                action.save()
    ...
    def create(self, request, *args, **kwargs):
        ...
        if contact:
            ...
            if (group.groupprofile.vendor_type == "Coordinator"):
                #check to see if this user is a coordinator/coordinator_mgr first
                if contact.user:
                    if not is_coordinator(contact.user):
                        return Response({'Error': "User must be a coordinator before adding to a coordination group."}, status=status.HTTP_400_BAD_REQUEST)
                    elif not is_coordinator_mgr(self.request.user):
                        if not is_in_lead_coord_team(self.request.user) or not group_admin:
                            return Response({'Error': "Please contact coordinator manager/group admin or member of lead coordination team to add users to this group."}, status=status.HTTP_400_BAD_REQUEST)
                else:
                    return Response({'Error': "This email does not belong to a current user. User must be a coordinator before adding to a coordination group."}, status=status.HTTP_400_BAD_REQUEST)                    
    ...
    def update(self, request, **kwargs):
        ...
        logger.debug("here in update")

        self.check_object_permissions(request, instance.group)
        data = request.data
        logger.debug(data)
        if data.get('group_admin'):
            if not instance.contact.user:
        ...
        if data.get('group_admin') == False and instance.contact.user == self.request.user:
            return Response({'detail':'You cannot remove your group admin permissions. If this is intentional, ask another group admin to remove your permissions or contact the coordination team.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if serializer.is_valid():
            ...
            for field, val in data.items():
                if (val != getattr(instance, field, None)):
                    if (field == "verified"):
                        if (val == True):
                            if (instance.group.groupprofile.vendor_type == "Coordinator"):
                                #check to see if this user is a coordinator/coordinator_mgr first
                                if instance.contact.user:
                                    if not is_coordinator(instance.contact.user):
                                        return Response({'detail': "User must be a coordinator before adding to a coordination group."}, status=status.HTTP_400_BAD_REQUEST)
                                    elif not is_coordinator_mgr(self.request.user):
                                        
                                        return Response({'Error': "Please contact coordinator manager to verify/add users to this group."}, status=status.HTTP_400_BAD_REQUEST)
                            ...
                            #EMAIL user to let them know they are now part of the group
                            ctx = {'group': instance.group.name, 'url': request.get_host()}
                            send_template_email("group_verified", [instance.contact.email], ctx)
                        else:
                            action = create_contact_action(f"unverified contact {instance.contact.email} for group {instance.group.name}", request.user, instance.contact)
                            _unverify_contact(instance)
...
class CreateContactView(LoginRequiredMixin, UserPassesTestMixin, FormView, FormMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.kwargs.get('group'):
            group = get_object_or_404(Group, groupprofile__uuid=self.kwargs['group'])
            context['action'] = reverse("cvdp:newcontact", args=[group.uuid])
        else:
        ...
        if form.is_valid():
            ...
            if errors.get('email'):
                ...
                if 'Email already exists' in str(errors['email']):
                    self.make_association(request, self.kwargs.get('group'))
    ...
    def make_association(self, request, pk=None):
        ...
        if pk:
            ...
            group = get_object_or_404(Group, groupprofile__uuid=pk)
            ca, created = ContactAssociation.objects.update_or_create(contact=contact, group=group,
