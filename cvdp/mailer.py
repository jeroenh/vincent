...
from cvdp.models import EmailTemplate, DailyEmailNotification
...
def get_daily_preference(user):
    logger.debug(user.userprofile.settings)
    s = user.userprofile.settings.get('email_daily', 1)
    if int(s) == 1:
        return True
    else:
        return False
...
def check_email_preferences(template_type, user):
    if template_type == EmailTemplate.CASE_EMAIL:
        ...
        #force these emails
        pref = ['1']
    elif template_type == EmailTemplate.TICKET_EMAIL:
        pref = user.userprofile.settings.get('email_new_tickets', ['1'])
    else:
        pref = []
...
def create_daily_notification(user, context):

    daily = DailyEmailNotification.objects.filter(user=user).first()

    case = context.get('prepend', "")
    url = context.get('url', "")
    template = context.get('template', "")
    message = context.get('message', "")

    if context.get('user_mentions'):
        if user.email in context['user_mentions']:
            #this user was tagged, so send immediately
            return False
    if context.get('group_mentions'):
        if user.email in context['group_mentions']:
            #this user's group was tagged, so ignore pref
            return False
    
    if daily:
        daily.notifications.append({'case': case, 'url': url, 'template': template, 'message': message})
        #add to the list

    else:
        e  = DailyEmailNotification(user=user,
                                    notifications=[{'case': case, 'url': url, 'template': template, 'message': message}])
        e.save()
    
    return True

def check_user_case_preferences(user, context):

    if get_daily_preference(user):
        if create_daily_notification(user, context):
            return True

    #add tracking/muted case settings
    return False
...
def send_templated_mail(template_name,
    ...
    if len(recipients) == 1:
        ...
        if user:
            ...
            logger.debug(t.template_type)
            if t.template_type in [EmailTemplate.CASE_EMAIL, EmailTemplate.POST_EMAIL]:
                if check_user_case_preferences(user, context):
                    return
...
def send_mail(subject,
    ...
    logger.debug('Sending email with subject "{}" to {!r}'.format(subject_part, recipients))
...
def mailer_send_email(subject, message, recipients, **attrs):
...
def send_daily_digest_mail(user, text, html):

    #url will be homepage
