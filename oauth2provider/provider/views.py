...
class PasswordChangeDoneView(LoginRequiredMixin, generic.RedirectView):
    ...
    def dispatch(self, request, *args, **kwargs):
        messages.success(self.request, 'Password change successful. Select My Dashboard to return to VINCE-NT.')
        return super().dispatch(request, *args, **kwargs)
...
class LoginView(auth_views.LoginView):
    ...
    def form_valid(self, form):
        ...
        if user_has_device(user):
            ...
            if nexturl:
                url = reverse("provider:mfa") + f"?next={nexturl}"
                logger.debug("redirecting to mfa provider with nexturl")
                return redirect(url)
            else:
...
class RegistrationView(generic.FormView):
    ...
    def form_valid(self, form):
        ...
        if settings.REQUIRE_MFA_AUTH:
            #redirect user to setup MFA before sending them back to VINCE-NT
...
class MFARequiredView(AccessMixin, generic.FormView):
    ...
    def form_valid(self, form):
        ...
        if next_url:
            if url_has_allowed_host_and_scheme(next_url, set(settings.ALLOWED_HOSTS), True):
                next_url = iri_to_uri(next_url)
                logger.debug("redirecting to provided next_url")
