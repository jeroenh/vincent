...
class SignUpForm(UserCreationForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if settings.RECAPTCHA_PUBLIC_KEY:
            ...
            self.captcha = settings.TURNSTILE_SITE_KEY
    ...
    def clean(self):
        ...
        if settings.RECAPTCHA_PRIVATE_KEY:
            ...
            recaptcha_response = self.data.get('cf-turnstile-response')
...
class TOTPTokenForm(OTPAuthenticationFormMixin, forms.Form):
    # tokens should be 6 or 8 digits
    ...
    def clean_otp_token(self):
        ...
        if len(token) < 6:
            self.add_error('otp_token', "Minimum 6 digits are required.")
            raise forms.ValidationError(" Invalid length for user code.")
        return token
