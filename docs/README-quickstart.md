...
3. Either Google RECAPTCHA or Cloudflare Turnstile must be configured for
form submissions to work correctly. Both have test keys available 
([Google](https://developers.google.com/recaptcha/docs/faq#id-like-to-run-automated-tests-with-recaptcha.-what-should-i-do) 
[Cloudflare](https://developers.cloudflare.com/turnstile/troubleshooting/testing/)).
4. Add the following two entries to your `/etc/hosts` file:
```
127.0.0.1   app.vincent.test
127.0.0.1   auth.vincent.test
```
5. Ensure the docker environment is ready (i.e. run `docker info` and no
errors should be reported).
6. If you have a docker.com account and have not done so already, use
...
base images. Note that some users report seeing IO errors when the build
is attempting to install some packages. Re-running the build seems to 
get past the issues. As of this writing, there is [currently still a bug 
filed in the Alpine repo](https://github.com/gliderlabs/docker-alpine/issues/348)
where some users encounter this problem. 
...
### V. Set up VINCE-NT Authentication
...
### VII. Optional Steps
  ...
  * Navigate to the "Groups" view of VINCE-NT and click on the vertical 3 dots to
  ...
  This will be the main coordination group for this VINCE-NT instance. Non-coordinator
  users will be able to send messages to this group.

## Next Steps
At this point, the VINCE-NT environment is ready for site configuration. Refer
