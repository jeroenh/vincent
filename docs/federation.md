...
VINCE-NT can now be configured to allow systems to receive and/or transfer
case information. This guide will assist an admin through the necessary
steps to support this feature.

VINCE-NT supports two types of connections. You can choose to allow one or both
...
cases to an external VINCE-NT system, you must have generated an API key on the external
...
1. A running VINCE-NT system. Follow the [README-quickstart](./README-quickstart.md) to setup your VINCE-NT environment(s).
...
### Configuration for accepting case transfers from another VINCE-NT system

1. If not already available, create a Group for the VINCE-NT system you intend
...
button.  You will need to provide this key to the admin of the other VINCE-NT instance.
...
4. Add the URL for the other VINCE-NT system.  This is important in both incoming and
outgoing connections, as VINCE-NT only allows access to the transfer API endpoints for
...
8. Cases transferred to your VINCE-NT instance can be viewed under "Triage." You can
view any threads transferred in your "Archived Threads." 

### Configuration for transferring cases to an external VINCE-NT instance (OUTGOING)

1. Similar to the previous instructions, create a Group for the system you intend
to share cases with.

2. If you have an existing account on the **other** VINCE-NT instance and are a
...
to use for transfers to the admin team of the **other** VINCE-NT instance before proceeding.
...
6. Add the URL for the other VINCE-NT system.
...
1. A running VINCE-NT system
2. A coordinator-level account
3. A reporting form has been created for your VINCE-NT system
...
resolution will indicate that the case has been transferred. Your VINCE-NT system retains
...
not be resent, but VINCE-NT will attempt to transfer all items for each category you choose. The
