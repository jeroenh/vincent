# VINCE-NT Configuration

VINCE-NT has several configuration options for user authentication
and registration.  Most settings can be configured with environment
variables, but there are a few that need to be set in vincent/settings.py.
...
#### DB_NAME (='vincent')
   This is the name of the database that is storing VINCE-NT data.

#### DB_PASS (='vincent')
   Set this to the password for the VINCE-NT database.

#### DB_PORT (='5432')
   This is the access port for the PSQL database.

#### DB_USER (='vincent')
   VINCE-NT uses a PostgresSQL database backend. Set this to the user that
   has access to the VINCE-NT database.
...
#### JOB_MANAGER
   ...
   all tasks will be run by the VINCE-NT Application.  Other options are:

   - cvdp.appcomms.async.AdviseWorker_Communicator
   - cvdp.appcomms.celery.CeleryRedis_Communicator

   The VINCE-NT worker option is the preferred option and can be used
...
#### ORG_NAME (='Test')
   ...
   and on automated email communications coming from VINCE-NT.
...
#### TURNSTILE_SECRET_KEY
   If using Cloudflare Turnstile for bot detection/prevention instead of ReCAPTCHA,
   set to the value of the Widget secret key

#### TURNSTILE_SITE_KEY
   If using Cloudflare Turnstile for bot detection/prevention instead of ReCAPTCHA,
   set to the value of the Widget site key
...
#### OAUTH_SERVER_BASEURL (='http://localhost:8080')
   If using the OAuth2 provider provided with VINCE-NT, this is the URL to the
...
### Email Settings
#### CONTACT_EMAIL (='')
   The email that will be used when sending automated emails from VINCE-NT.
...
#### EMAIL_HEADERS (={})
   Optionally, append additional email headers when sending email notifications from VINCE-NT.
...
#### DISCLOSURE_POLICY_LINK (="")
   Optional link to your disclosure policy that can be found in the footer of VINCE-NT.

## Two-factor Authentication (2FA)

   By default, VINCE-NT requires all local users to enable 2FA upon registering with the
   application.  If you do not wish to require 2FA, remove 'authapp.middleware.Require2FAMiddleware'
   from the MIDDLEWARE setting in vincent/settings.py
