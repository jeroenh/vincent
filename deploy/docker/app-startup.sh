...
if [ "X${WSGI_APP}" = "Xvincent.wsgi" ]; then
...
# don't run if we're a worker, as the main container should do these things
if [ "X${IS_WORKER_APP}" = "X" ]; then
    # these should run for both oauth and app containers
    python manage.py makemigrations --noinput
    python manage.py migrate --noinput
    python manage.py createsuperuser --noinput
    if [ "X${WSGI_APP}" = "Xvincent.wsgi" ]; then
        # this is only valid for the app container
        python manage.py loadinitialdata
    fi
fi

# collect static for app and oauth containers
if [ "X${IS_WORKER_APP}" = "X" ]; then
    if [ "X${NOCLEAR}" = "X" ]; then
...
if [ "X${RUN_TESTS_ONLY}" = "X${WSGI_APP}" ]; then
    # run the tests instead, but only for the non-worker container!
    if [ "X${IS_WORKER_APP}" = "X" ]; then
        python manage.py test
    else
        # else... we just hang out indefinitely (we can't go silently into the night without flagging an abort)
        while [ 1 ]; do
            sleep 1
        done
    fi
