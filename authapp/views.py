...
class PreferencesView(LoginRequiredMixin, PendingTestMixin, FormView):
    ...
    def form_valid(self, form):
        logger.debug(f"{self.__class__.__name__} post: {form.cleaned_data}")
        s = self.request.user.userprofile
        settings = s.settings
...
#            # Query for all SocialApp instances associated with the current site
#            # This will return a list of configured providers
#            social_apps = SocialApp.objects.filter(sites__in=[current_site])
#            logger.debug(social_apps)
#            # Extracting the provider names (e.g., 'google', 'github')
#            for app in social_apps:
#                if hasattr(settings, 'USE_PROVIDER'):
#                    if app.provider == settings.USE_PROVIDER:
#                        continue
#                provider_list.append({'id': app.id, 'provider': app.provider, 'name': app.name})
#        
#            logger.debug(provider_list)
#            context['providers'] = provider_list
#        except:
#            logger.debug(traceback.format_exc())
#            #likely no providers available so ignore
#            pass
#            
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.request.session.get("USERNAME"):
            ...
            if not is_user_email_verified(user, user.email):
                ...
                emailaddress.send_confirmation(request, signup=True)
                #below is old way
