...
describe("Post Component", () => {
    ...
    it("should pin/unpin post", async () => {
        ...
        mock.onGet("http://localhost:8000/cvdp/api/cases/312650/").reply(
            200,
            data['case']
        );
        mock.onGet(
            "http://localhost:8000/cvdp/api/case/312650/participants/?role=owner"
        ).reply(200, data['owners']);
        
        mock.onGet("http://localhost:8000/cvdp/api/case/312650/user/")
            .reply(200, data['usercasestate'])


        mock.onGet("http://localhost:8000/cvdp/api/case/312650/threads/")
        ...
        const { container } = render(
            <MemoryRouter initialEntries={["/cvdp/cases/312650"]}>
            <CaseApp />
            </MemoryRouter>
        );
        ...
        await waitFor(() => {
            ...
            expect(container.querySelector(".note_input")).toBeInTheDocument();
        })
        ...
        mock.onPatch("http://localhost:8000/cvdp/api/case/thread/post/218/")
        .reply(200, []);
        mock.onGet("http://localhost:8000/cvdp/api/case/thread/149/posts/?pinned=1")
    ...
    it("should edit post and show edits if available", async () => {
        ...
        mock.onPatch("http://localhost:8000/cvdp/api/case/thread/post/218/")
        .reply(200, []);
        mock.onGet("http://localhost:8000/cvdp/api/case/thread/post/218/")
        ...
        await waitFor(() => {
            ...
            const editors =  container.getElementsByClassName('note_input')
    ...
    it("should allow reactions to posts", async () => {
        ...
        await waitFor(() => {
            expect(container.querySelector(".post-like")).toBeInTheDocument();
        })

        await waitFor(() => {
            const postreact = container.getElementsByClassName('post-like');
            user.click(postreact[0]);
        })
	...
	mock.reset();
        ...
        mock.onPost("http://localhost:8000/cvdp/api/case/thread/post/218/react/")
        .reply(202, [{"count":1,"reaction":"like","users":"emily"}]);

	
        await waitFor(() => {
            expect(container.querySelector(".post-like-menu")).toBeInTheDocument();
            user.click(screen.getByTestId("thumbs-up"));
        })

	
	await waitFor(() => {
	    expect(container.querySelector('.post-react')).toBeInTheDocument();
	...
	await waitFor(() => {
	    const reaction = container.getElementsByClassName('post-react');
	    user.click(reaction[0]);
	});
	
	/* remove reaction */
	await waitFor(() => {
	    expect(container.querySelector('.post-react')).not.toBeInTheDocument();
	})
