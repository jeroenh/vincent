...
import {Table, Stack, Card, Badge, DropdownButton, Dropdown, Alert, Accordion, Row, Col, Button, Form} from 'react-bootstrap';
...
import VulsAffected from "./VulsAffected";
...
const VulAddForm = (props) => {
    ...
    const [activeCve, setActiveCve] = useState(searchParams.get('cve') || null);
    ...
    const [showVulsAffected, setShowVulsAffected] = useState(false);
	...
	if (searchParams.get('cve')) {
            let f = vuls.find(v => v.cve === searchParams.get('cve'));
            if (f) {
                setOpenCve(vuls.indexOf(f).toString());
	    }
        }
    ...
    const hideVulsAffected = () => {
	setShowVulsAffected(false);
    }
    ...
    const hidePopulateModal = (err = null) => {
    ...
    const pullCVEInfo = async (e, cveid=null) => {
	/*get user CVE account */
	if (cveid) {
	    setVulCVE(cveid);
	}
    ...
    const downloadCVEJson = (vul) => {

	let cveapi = new CVEAPI();
	let vuljson = cveapi.generateCVEJson(vul);

	if (vuljson) {
	
	    var dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(vuljson, null, 2));
            const a = document.createElement('a');
            a.setAttribute("href",     dataStr);
            a.setAttribute("download", `${vul.vul}` + ".json");
            document.body.appendChild(a);
            a.click();
            a.remove();
	} else {
	    setErrorMessage("CVE is missing information. Add required information before downloading JSON.");
	    setDisplayErrorModal(true);
	}
	...
	if (props.vuls) {
	    if (activeCve) {
		let f = props.vuls.find(v => v.cve === activeCve);
		if (f) {
                    setOpenCve(props.vuls.indexOf(f).toString());
		}
            }
	    setVuls(props.vuls)
	}
	...
	isLoading ?
 	...
 	:
		...
		<div className="d-flex align-items-center justify-content-between" ref={navRef}>
		    ...
		    {props.updateVuls && props.user?.role === "owner" &&
		     <>
			 {vuls.length > 0 ?
			  <DropdownButton variant="btn p-0"
					  id="vul-dropdown"
					  title={<i className="bx bx-dots-vertical-rounded" title="Manage Vulnerabilities"></i>}
			  >
                              <Dropdown.Item eventKey='add' onClick={()=>(editVulNow(null), setShowEditVulModal(true))} >Add Vulnerability</Dropdown.Item>
			      <Dropdown.Item eventKey='edit' onClick={()=>(editVulNow(vuls), setShowEditVulModal(true))}>Edit Vuls</Dropdown.Item>
			      <Dropdown.Item eventKey='bulk' onClick={(e)=>(navigate("vuls", {state: {caseInfo: caseInfo, reqUser: props.user, vuls: vuls}}))}> Table View </Dropdown.Item>
			  </DropdownButton>
			  :
			  <Button type="button" className="btn btn-primary" onClick={()=>(editVulNow(null), setShowEditVulModal(true))}>
			      Add Vul
			  </Button>
			 }
		     </>
		...
		{vuls.length > 0 &&
		 <Accordion defaultActiveKey={[openCve] || null} activeKey={[openCve] || null} onSelect={(e)=>(openCve == e ? setOpenCve(null) : setOpenCve(e))}>
			 ...
			 return (
				...
				<Accordion.Header>
				    ...
				    {vul.publish ? "":<i title="Marked as don't publish" className="fas fa-exclamation-triangle link-danger me-2"></i>}
				    {vul.vul}: {vul.title ? vul.title : vul.description}
				</Accordion.Header>
				<Accordion.Body>
				    
				    {vul.publish ? "" :
				     <Alert variant="danger">
				     This vulnerability is marked as not publishable and will not be published in any advisory or shared with case participants.</Alert>
				    }
				    ...
				    {props.updateVuls &&
					 ...
					 <VulApprovalApp
					     vul = {vul}
					     preview={showPreviewApp}
					     update={props.updateVuls}
					 />
				     </div>
				    }
				    ...
				    {vul.tags && vul.tags.length > 0 &&
					 ...
					 <Stack direction="horizontal" gap={2}>
						 ...
						 return (
						     <Badge key={`tag-${index}`} bg="info">{t}</Badge>
						 )
					     })}
					 </Stack>
				     </div>
				    }
				    {vul.cve_tags && vul.cve_tags.length > 0 &&
				     <div className="mb-2">
                                         <b>CVE CNA Tags:</b>{" "}
                                         {vul.cve_tags.map((t, index) => {
                                         return (
                                             <Badge key={`cvetag-${index}`} bg="secondary">{t}</Badge>
                                         )
                                     })}
                                     </div>
                                    }
				    ...
				    {vul.references && vul.references.length > 0 &&
                                     ...
                                     <ul className="list-unstyled">
                                         {vul.references.map((t, index) => {
                                             return (
                                                 <li key={index}>{t.url} {t.summary && `(${t.summary}}`} {t.tags?.map((tag, idx) => (<Badge bg="primary" className="mx-1" key={`reftag-${index}-${idx}`}>{tag}</Badge>))}</li>
                                             )
                                         })}
                                     </ul>
                                     </div>
                                    ...
                                    {vul.acknowledgments && vul.acknowledgments.length > 0 &&
                                     <div>
                                     <b>Acknowledgments:</b><br/>
                                     <ul className="list-unstyled">
                                         {vul.acknowledgments.map((t, index) => {
						 ...
						 return (
						  ...
						  return (
                                                      <li key={index}>{t.names} {t.organization && `(${t.organization})`}</li>
                                                  )
					     }
				    ...
				    {vul.affected_products.length > 0 &&
				     <div className="mb-3">
					 {props.updateVuls ?
					  <b>Affected Products:</b>
					  :
					  <b>Your Affected Products:</b>
					 }
					 <br/>
					 <b>{vul.affected_products.length}</b> affected product{vul.affected_products.length > 1 && `s`}. <Button className="btn btn-primary" size="xs" onClick={(e)=>{e.preventDefault(), setScoreVul(vul), setShowVulsAffected(true)}}><i className="fas fa-eye me-1"></i> View all</Button>
					 <VulsAffected
					     showModal = {showVulsAffected}
					     hideModal = {hideVulsAffected}
					     vul = {scoreVul}
					 />
				     </div>
				    }
				    {vul.cvss?.map((cvss, index) => (
					<div className="mb-3" key={`cvss-${cvss.version}`}>
					    <div>
						<b>CVSS V{cvss.version}:</b> {cvss.vectorString}
					    </div>
					    <div>
						<b>CVSS Base Score:</b> {cvss.score}
					    </div>
					    <div>
						<b>CVSS Severity:</b> <CVSSSeverityBadge
					...
					>
					    ...
					    {vul.cve &&
						 ...
						 {vul.publish &&
						  <>
						      <Dropdown.Item eventKey="publish" onClick={(e)=>(setPublishVul(vul), setShowPublishModal(true))}>Publish CVE</Dropdown.Item>
						      <Dropdown.Item eventKey="download" onClick={(e)=>downloadCVEJson(vul)}>Download CVE JSON</Dropdown.Item>
						  </>
						 }
						 <Dropdown.Item eventKey="sync" onClick={(e)=>(setEditVul(vul), setVulCVE(vul.vul), pullCVEInfo())}>Sync CVE</Dropdown.Item>
						 </>
					    }
					    {vul.affected_products && vul.affected_products.length > 0 &&
					     <Dropdown.Item eventKey="vex" href={`/cvdp/api/vul/${vul.id}/vex/`}>Download VEX</Dropdown.Item>
					    }
					</DropdownButton>
				    </div>
				    }
