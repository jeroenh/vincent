...
const DesignForm = () => {
	...
	await formapi.getForm(id).then((response) => {
	    setTheForm(response);
	}).catch(err => {
	    setError(err.message);
	});
	
	await formapi.getQuestionTypeOptions(id).then((response) => {
	    setQuestionOptions(response['actions']['POST']['question_type']['choices'])
	}).catch(err => {
	    setError(err.message);
	});
	
	await formapi.getQuestions(id).then((response) => {
	...
	if (error == false) {
		...
		if (editQuestion) {
		    ...
		    formapi.addQuestion(id, formDataObj).then((response) => {
	...
	await formapi.removeForm(id).then((response) => {
	    window.location.href="/cvdp/manage/forms";
	}).catch(err => {
	    setError("Unable to remove this form.");
	});
    };
	...
	isLoading ?
 	...
 	:
	    <>
		{showRemove &&
		 <Alert variant="danger"><p>Are you sure you want to remove this Form?  This cannot be undone.</p>
		     <div className="d-flex align-items-center gap-3">
			 <Button variant="secondary" onClick={(e)=>setShowRemove(false)}>Cancel</Button>
			 <Button variant="danger" onClick={(e)=>removeForm()}>Remove</Button>
		     </div>
		...
		<Card>
			...
			<Accordion>
				...
				return (
					...
					<Accordion.Header>
					    {question.id == editQuestion ? <i className="fas fa-edit p-2" title="Edit Question"></i> : "" }
					    {question.question}
					</Accordion.Header>
					<Accordion.Body>
					    <div className="p-3">
						...
						<div className="actions text-end">
						    <Button className="p-2 m-2" variant="outline-secondary" onClick={(e)=>editQuestionNow(question)}><i className="fas fa-edit" title="Edit Question"></i></Button><Button variant="danger" className="p-2" onClick={(e)=>removeQuestion(question.id)}><i className="fas fa-trash" title="Remove Question"></i></Button>
						</div>
					    </div>
					</Accordion.Body>
				    </Accordion.Item>
