...
import {Alert, OverlayTrigger, Popover, Card, Col, Dropdown, DropdownButton, Button, Form, InputGroup} from 'react-bootstrap';
...
const ParticipantTable = () => {
	...
	let owners = selectedRows.filter(item => item.original.role === "owner");
	if (owners.length > 0) {
	    setError("Case owners must be unassigned from the case page.");
	    setDisplayErrorModal(true);
	}

	if (owners.length == selectedRows.length) {
	    return;
	}
	
	selectedRows.map(item => {
	    if (item.original.role !== "owner") {
	...
	await threadapi.notifyCaseParticipants({'case': caseInfo.case_id}, formField).then((response) => {
	    ...
	    console.log(err);
	    setError(`An error occurred while notifying participants: ${err.response.data.message}`);
	    setDisplayErrorModal(true);
	});
	...
	},
	{
	    ...
	    Header: 'Viewed Case',
	    accessor: row => {
		if (row.viewed?.length > 0) {
		    return (
			<OverlayTrigger
                            placement="bottom"
                            overlay={<Popover><Popover.Header className="text-center border-bottom pb-2">Viewed By</Popover.Header><Popover.Body className="text-center m-2">{row.viewed.map((u, index)=>(<div key={`part-u${index}`}>{u.user}: ({format(new Date(u.date_viewed), 'yyyy-MM-dd')})</div>))}</Popover.Body></Popover>} >
			    <a href="#" onClick={(e)=>e.preventDefault()}><i className="fas fa-eye alert-success"></i></a>
			</OverlayTrigger>
			)
		} else {
		    return (
			<><i title="Case not viewed" className="fas fa-eye-slash warningtext"></i></>
		    )
		}
	    }
	},
	{
		...
		const onItemClick = value => {
			...
			/* does this essentially unassign the case? */
			setError("Case owners must be unassigned from the case page.");
			setDisplayErrorModal(true);
			return;
		    }
	...
	caseInfo ?
		...
		<Card>
			...
			{caseInfo && caseInfo.status === "Pending" &&
			 <Alert variant="warning">This case is currently in <b>Pending</b> state. Users cannot be notified until case state has been changed to <b>Active</b>.</Alert>
			...
			<div className="d-flex align-items-start justify-content-between mt-2 gap-2 flex-column flex-sm-row">    
