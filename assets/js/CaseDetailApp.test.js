...
describe("Case Detail Component", () => {
    ...
    it("should render report and allow for edits", async () => {
       ...
       .reply(200, [])
       mock.onGet("http://localhost:8000/cvdp/api/cases/310363/")
       ...
       .onAny().reply(200, []);
        ...
        await waitFor(() => {
            expect(screen.getAllByText(/Case Title/i)).toHaveLength(1);
            expect(screen.getAllByText(/Summary/i)).toHaveLength(2);
            fireEvent.change(screen.getByTestId('case_title_input'), {
                target: 
                {value: 'My Case Title'}
            });
    ...
    it("should render vuls and allow for edits", async () => {
        ...
        mock.onGet('http://localhost:8000/cvdp/api/case/thread/149/participants/')
        .reply(200, data['caseparticipants'])
        mock.onGet('http://localhost:8000/cvdp/api/case/310363/vuls/')
        .reply(200, data['vuls'])
        .onAny()
        .reply(200, []);
        ...
        await waitFor(() => {
            ...
            expect(screen.getByText('VINCE-NT Tags')).toBeInTheDocument();
        ...
        mock.onPatch("http://localhost:8000/cvdp/api/vul/109")
        .reply(200, [])
        ...
        await waitFor(() => {
            expect(screen.getByText(/Edit Vulnerability CVE-9999-21128/i)).toBeInTheDocument();
            expect(screen.getAllByRole('button', {name:/Sync CVE/i})).toHaveLength(2);
	    ...
	    "cvss": [],
            ...
            "affected_products": [],
            "case": "CASE#312650",
            "url": "/cvdp/cases/312650"
        })

	let currentYear = new Date().getFullYear();
        mock.onGet("http://localhost:8000/cvdp/api/manage/cve/account/?active=true")
        ...
        mock.onGet("http://localhost:8000/cvdp/api/cases/310363/")
        .reply(200, data['case'])
        mock.onPost('http://localhost:8000/cvdp/api/case/310363/vuls/')
        .reply(200, [])
        mock.onGet('http://localhost:8000/cvdp/api/case/310363/vuls/')
	    ...
	    /* changed from 1 to 2 (nov 2025) as we now keep track of reserved CVEs within VINCE-NT */
    ...
    it("should render vuls and allow the user to score them", async () => {
        ...
        await waitFor(() => {
            expect(screen.getAllByText(/Vul1/i)).toHaveLength(2);
            const option_btns = screen.getAllByRole('button', {name:/Options/});

	    /* if you run this test by itself using it.only() this will only have 2 but
	       because these tests are cumulative and we added a vul in one of them, it returns 3 if you
	       run all the tests, likewise in below check */
        ...
        await waitFor(() => {
            ...
            user.click(screen.getByRole('button', {name:/Score/i}));
        });

        await waitFor(() => {
            expect(screen.getByText(/Score the vulnerability/i)).toBeInTheDocument();
	    /* multiple CVSS versions */
            expect(screen.getAllByRole('tab', {name:/CVSS/})).toHaveLength(2);
        ...
        await waitFor(() => {
	    expect(screen.getByTestId("decision-tree-select")).toBeInTheDocument()
            //expect(screen.getByLabelText(/Decision Tree/)).toBeInTheDocument();
	...
	*/
	...
	/*
        ...
        await waitFor(() => {
	    /* ssvc and cvss 4 have automatable buttons */
            expect(screen.getAllByRole('button', {name:/Automatable/i})).toHaveLength(2);
	...
	await waitFor(() => {
        ...
        mock.onPost("http://localhost:8000/cvdp/api/case/vul/109/ssvc/")
        .reply(200, [])
        mock.onGet('http://localhost:8000/cvdp/api/case/310363/vuls/')
        .reply(200, revised_vuls)
        /*mock.onGet("http://localhost:8000/cvdp/case/vul/109/cvss/")
        ...
        await waitFor(() => {
            ...
            user.click(screen.getByRole('button', {name:/Publish/i}));
        });
    ...
    it("should render case status and components", async () => {
        ...
        mock.onGet('http://localhost:8000/cvdp/api/case/310363/vuls/')
        .reply(200, data['vuls'])
        mock.onGet('http://localhost:8000/cvdp/api/case/310363/components/')
        ...
        await waitFor(() => {
            /*add a new status */
            expect(screen.getAllByText(/dsftklsjdf/)).toHaveLength(2);
            user.click(screen.getAllByText(/dsftklsjdf/)[0]);
        });
        ...
        /* Edit Status */
        await waitFor(() => {
            expect(screen.getByText(/Edit Component Status/i)).toBeInTheDocument();
            expect(screen.getByTestId("status-cancel")).toBeInTheDocument();
            user.click(screen.getByTestId("status-cancel"));
        });
        
        await waitForElementToBeRemoved(() => screen.getByText(/Edit Component Status/i))
        ...
        mock.onDelete("http://localhost:8000/cvdp/api/case/component/80/status/")
        .reply(200, [])
        mock.onGet('http://localhost:8000/cvdp/api/case/310363/components/')
        .reply(200, data['compstatus'])
        ...
        await waitFor(() => {
            user.click(screen.getAllByText(/something/i)[0]);
	...
	mock.onGet("http://localhost:8000/cvdp/api/cases/310363/").reply(
        ...
        await waitFor(() => {
            ...
            expect(screen.getByText(/Add Status/i)).toBeInTheDocument();
            expect(screen.getByTestId('status-view-all')).toBeInTheDocument();
	    user.click(screen.getByTestId('status-view-all'));
        });

        await waitFor(() => {
            expect(screen.getByPlaceholderText("Search Components")).toBeInTheDocument();
	    expect(screen.getAllByText(/dsftklsjdf/i)).toHaveLength(2);
            user.click(screen.getAllByText(/dsftklsjdf/)[1])
        ...
        const rm_btns = container.querySelectorAll(".rm-status-btn");
        expect(rm_btns).toHaveLength(3);
        fireEvent.click(rm_btns[2]);
    ...
    it("should render err if permission denied", async () => {
        ...
        const { container } = render(
            <MemoryRouter initialEntries={["/cvdp/cases/310363/status"]}>
            <CaseApp />
            </MemoryRouter>
        );
