...
describe("Case App", () => {
    ...
    it("should render search cases by default", async () => {
        ...
        mock.onGet("http://localhost:8000/cvdp/api/cases/?&status=0&status=1&page=1").reply(
            200,
            mockcaseresults
        );

	mock.onGet("http://localhost:8000/cvdp/api/case/metadata/").reply(
	    200,
	    data["metadata"]
	);
        ...
        render(
            <MemoryRouter
                initialEntries={["/cvdp/cases", "/cvdp/cases/:id"]}
                initialIndex={0}
            >
                <CaseApp />
            </MemoryRouter>
	    ...
	    //should show all states, and Owner options for user, all, and unassigned
	...
	mock.onGet("http://localhost:8000/cvdp/api/cases/312650/metdata/").reply(
	    200,
	    data["metadata"]
	);
    ...
    it("should render err if url contains non existent case id", async () => {
        /*let rep = nock("http://localhost:8000/cvdp")
            .get("/api/cases/123456/")
            .once()
            .reply(404, new Error("not found"))*/
        mock.onGet("http://localhost:8000/cvdp/api/cases/123456/").reply(
            404,
            new Error("not found")
        );

        render(
            <MemoryRouter initialEntries={["/cvdp/cases/123456"]}>
                <CaseApp />
            </MemoryRouter>
        );
    ...
    it("should render status table if url contains staus", async () => {
        ...
        mock.onGet("http://localhost:8000/cvdp/api/cases/312650/status/")
            .reply(200, [])
            .onAny()
            .reply(200, []);

        render(
            <MemoryRouter initialEntries={["/cvdp/cases/312650/status"]}>
                <CaseApp />
            </MemoryRouter>
        );
	    ...
	    /* get case, get components, get user */
    ...
    it("should render err if permission denied on case status", async () => {
        ...
        render(
            <MemoryRouter initialEntries={["/cvdp/cases/123456/status"]}>
                <CaseApp />
            </MemoryRouter>
        );
