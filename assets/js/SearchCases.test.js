...
describe("Search Cases Component", () => {
	...
	mock.onGet("http://localhost:8000/cvdp/api/cases/?&status=0&status=1&page=1")
            .reply(200, mockcaseresults)
            .onGet("http://localhost:8000/cvdp/api/cases/?search=something&status=0&status=1&page=1")
            .reply(200, []);
	...
	mock.onGet("http://localhost:8000/cvdp/api/cases/?&status=0&status=1&page=1")
            .reply(200, mockcaseresults)
            .onGet("http://localhost:8000/cvdp/api/cases/?search=Airpods&status=0&status=1&page=1")
            .reply(200, mockcaseresults)
            .onGet(
                "http://localhost:8000/cvdp/api/cases/?search=Airpods&status=2&page=1"
            )
            .reply(200, [])
            .onAny()
            .reply(200, mockcaseresults);
	    ...
	    const activecheckbox = screen.getByTestId("check-Active");
	    const pendingcheckbox = screen.getByTestId("check-Pending");
	    /*uncheck pending and active*/
	    ...
	    /* get cases, get active, get pending, get inactive, search + 1 */
            expect(mock.history.get.length).toBe(7);
        });
    });
});
