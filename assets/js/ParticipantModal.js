...
const ParticipantModal = (props) => {
    ...
    const addGroupsByTag = async (tag, remove) => {

	await contactapi.getGroupsByTag(tag).then(response => {
		...
		response.results.forEach(g => {
		    if (!selected.find(x => x.uuid === g.uuid)) {
			const test = {'name': g.name, 'logo': g.photo, 'color': g.logocolor, 'uuid': g.uuid};
	...
	if (error) {
	    const el = document.querySelector('.modalheader');
	...
	if (event.target.checked) {
	    if (name === uuid) {
		addGroupsByTag(name, false);
		setTagsSelected(tagsSelected => [...tagsSelected, name]);
	    } else {
	    ...
	    if (name === uuid) {
		addGroupsByTag(name, true);
		setTagsSelected(current => current.filter((select) => select !== name));
	    } else {
    ...
    return (
        <Modal show={props.showModal} onHide={props.hideModal} size="lg" centered backdrop="static">
            <Modal.Header closeButton className="border-bottom modalheader">
		...
		<>
		{error &&
		 <Alert variant="danger" id="errormsg">{error}</Alert>
		}
		</>
		...
		<Row>
			...
			<InputGroup className="w-100">
			    ...
			    <Form.Control
				placeholder="Search for Contacts or Groups by name or tag"
				aria-label="Search for Contacts or Group by name or tag"
			...
			<div className="mt-3">
			    {suggested.length > 0 ?
				 ...
				 suggested.map((s, index) => {
				     let isChecked=false;
				     let isDisabled="";
				     if (selected.some(e => e.uuid == s.uuid) || tagsSelected.includes(s.name)) {
					 isChecked=true;
				     }
					 ...
					 return (
						 ...
						 <div className="d-flex align-items-center gap-2"> 
							 ...
							 tag = {true}
						     />
						     <Badge bg="primary">{s.name}</Badge>
						 </div>
						 <Form.Check
                                                     disabled={isDisabled}
                                                     type='checkbox'
                                                     id={s.name}
						     aria-label={`Add ${s.name} to case`}
                                                     checked={isChecked}
                                                     onChange={(event)=>handleCheck(event, s.name, s.photo, s.logocolor, s.uuid)}
						 />    
                                             </div>
					 )
				     } else {
					 ...
					 <div className="d-flex align-items-center justify-content-between mt-2" key={index}>
						 ...
						 <DisplayLogo
						     ...
						     color={s.logocolor}
						 />
						 <h4 className="partmodal-name">{s.name}</h4>
						 {s.type === "contact" &&
						  <>
						      {s.groups.length > 0 &&
						       `(${s.groups.join(", ")})`
