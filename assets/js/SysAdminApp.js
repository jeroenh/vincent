...
import VulAttributesManager from "./VulAttributesManager";
import DisplayLogo from "./DisplayLogo";
import PermissionsApp from "./PermissionsApp";
import CoordTeamApp from "./CoordTeamApp";
...
const SysAdminApp = () => {
    ...
    const enableConnection = (val) => {
        ...
        adminapi
            .updateConnection(val, formDataObj)
            .then((response) => {
                fetchInitialData();
            })
            .catch((err) => {
    ...
    const submitConnection = (event) => {
        ...
        if (editConnection) {
            adminapi
                .updateConnection(editConnection, formDataObj)
                .then((response) => {
                    setShowForm(false);
                    setGroup("");
                    setOutgoingURL("");
    ...
    return apiError ? (
        ...
        >
            <Nav variant="pills" className="mb-3">
                ...
                <Nav.Item key="teams">
                    <Nav.Link eventKey="teams">Coordination Teams</Nav.Link>
                </Nav.Item>
                {/*<Nav.Item key="perms">
		...
		<Nav.Item key="attributes">
                    <Nav.Link eventKey="attributes">Vul Attributes</Nav.Link>
                </Nav.Item>
            </Nav>
            <Tab.Content id="admin-fns" className="p-0">
                <Tab.Pane eventKey="fed" key="fed">
                    <Card className="mb-4">
                        ...
                        <Card.Body>
							...
							Group is required. Provide valid group.
                                                    </Form.Text>
                                                )}
                                            </Form.Group>
                                            <Form.Group className="mb-3" controlId="URL">
                                                ...
                                                {invalidURL && (
                                                    <Form.Text className="error">
							URL required. Provide valid URL or system.
                                                    </Form.Text>
                                                )}
                                            </Form.Group>
                                            <Form.Group className="mb-3" controlId="share key external">
                                                <Form.Label className="mb-0">
                                                    Share Key (external)<span className="required">*</span>
                                                </Form.Label>
                                                <Form.Text>
                                                    API key generated by system
                                                    you intend to share with
                                                </Form.Text>
                                                <Form.Control
                                                    name="external_key"
                                                    isInvalid={invalidOutgoing}
                                                    value={outgoingKey}
                                                    onChange={(e) =>
                                                        setOutgoingKey(
                                                            e.target.value
                                                ...
                                                {invalidOutgoing && (
                                                    <Form.Text className="error">
                                                        Either external key OR
                                                        incoming API key is
                                                        required.
                                                    </Form.Text>
                                                )}
                                            </Form.Group>
                                            <Form.Group className="mb-3" controlId="internal api key">
                                                <Form.Label className="mb-0">
                                                    Allow Transfers from
                                                    (internal)<span className="required">*</span>
                                                </Form.Label>
                                                <Form.Text>
                                                    Last 4 digits of API key to
                                                    allow case transfer requests
                                                    (see API tab for Group)
                                                </Form.Text>
                                                <Form.Control
                                                    name="incoming_api_key"
                                                    isInvalid={invalidIncoming}
                                                    value={incomingKey}
                                                    onChange={(e) =>
                                                        setIncomingKey(
                                                            e.target.value
                                                ...
                                                {invalidIncoming && (
                                                    <Form.Text className="error">
                                                        Either the internal API key OR
                                                        outgoing API key is
                                                        required.
                                                    </Form.Text>
                                                )}
                                            ...
                                            >
                                                <Card.Header as="h5" className="d-flex justify-content-between">
                                                    ...
                                                    <DropdownButton
                                                        variant="btn p-0"
                                                        title={
                                                            <i className="bx bx-dots-vertical-rounded" title="manage connection"></i>
                                                        }
                                                    >
                                                        <Dropdown.Item
                                                            eventKey="edit"
                ...
                <Tab.Pane eventKey="teams" key="teams">
                    <CoordTeamApp />
                </Tab.Pane>
                {/*<Tab.Pane eventKey="perms" key="perms">
                    <PermissionsApp />
                    </Tab.Pane>*/}
		...
		<Tab.Pane eventKey="attributes" key="attributes">
                    <VulAttributesManager />
                </Tab.Pane>
                <DeleteConfirmation
