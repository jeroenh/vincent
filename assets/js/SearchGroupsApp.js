...
import {Card, Badge, DropdownButton, Dropdown, ListGroup, InputGroup, Form, Row, Col, Table, Accordion, Alert, Button} from 'react-bootstrap';
...
const SearchGroupsApp = () => {
	...
	<>
	    <Row>
		<Col lg={9} md={9} sm={12}>
		    <h4 className="fw-bold py-3 mb-4"><span className="text-muted fw-light">Contacts /</span> Contact Search</h4>
		</Col>
		<Col lg={3} sm={12} md={3} className="text-end mb-4">
		    <ContactManageDropdown home={true} />
		</Col>
	    </Row>
	    <Row>
		<Col lg={12}>
		...
		<Col lg={8} md={6} sm={12}>
			...
			<Card.Header as="h5" className="pb-0">
				...
				<DropdownButton variant="btn p-0 groupactions" role="menuitem"
						title={<i title="contact management options" className="bx bx-dots-vertical-rounded"></i>}
				>
				    
				    <Dropdown.Item eventKey='group' onClick={(e)=>setAddGroupModal(true)} title="Add Group">Add Group</Dropdown.Item>
				    <Dropdown.Item eventKey='contact' onClick={(e)=>setAddContactModal(true)} title="Add Contact">Add Contact</Dropdown.Item>
			...
			<Card.Body>
				...
				/>
				<Button variant="btn btn-outline-secondary" id="button-addon2" type="submit" title="Search Contacts">
				    <i className="fas fa-search" title="Search Contacts"></i>
				</Button>
			    </InputGroup>
				 ...
				 <div className="table-responsive text-nowrap mb-4">
					 ...
					 <tbody className="table-border-bottom-1">
						 ...
						 return(
							 ...
							 <td>{group.type == "Group" ? 
							     <Link to={`${group.uuid}`}>{group.name ?
										       `${group.name}` :
										       `${group.email}`
										      }</Link>
							      :
							      <Link to={`/cvdp/contact/${group.uuid}`}>{group.name ?
													`${group.name}` :
													`${group.email}`
												       }</Link>
							     }
							 ...
							 <td>{group.user_name &&
							     ...
							     {group.tags?.length > 0 &&
								  ...
								  {group.tags.map((tag, idx) => (
								      <Badge key={`tag-${idx}`} className="mx-2" bg="primary">{tag}</Badge>
								  ))}
							      </>
							     }
							 </td>
		...
		<Col lg={4} md={6} className="d-none d-md-block">
		    <Card className="card-max-height-1000">
			<Card.Header as="h5" className="pb-1">
                            <Card.Title>Recent Activity</Card.Title>
			</Card.Header>
			<Card.Body className="p-2 card-body-max-height">
                            {activityLoading ?
