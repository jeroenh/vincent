...
const CVSSv4App = forwardRef((props, ref) => {
		...
		<Col lg={9} md={12}>
		    ...
		    >
                    {Object.keys(props.options).map((option, index) => {
                        ...
                        return (
			    <ToggleButton
				id={`toggle-btn-${props.label}-${props.options[option].value}`}
	...
	setExisting(false);
	if (props.metrics) {
	    if (props.metrics.version == "4.0") {
		if ('vectorString' in props.metrics) {
		    setExisting(true);
		    setScoreVector(props.metrics['vectorString'])
		    let vec = new CVSS40(props.metrics['vectorString']);
		    let score = vec.Score();
		    setScore(score);
		    let severity = CVSS40.Rating(score);
		    setMacro(vec.macrovector());
	...
	<>
		...
		<h4>Common Vulnerability Scoring System 4.0 Calculator</h4>
		<Button variant="secondary" onClick={(e)=>(resetVector(), setUserInput(true))}>Reset</Button>
	    </div>
	    <p className="lead mb-1">{scoreVector}</p>
	    <p className="lead mb-1">CVSS v4.0 Score: <b>{score}</b>/ <Badge bg={severityColor}>{severity || `None`}</Badge> <Button variant="icon" onClick={(e)=>toggleShowDetail()}><i className={showDetail ? "fas fa-caret-square-up" : "fas fa-caret-square-down"} title="Show More/Less"></i></Button></p>
	    {showDetail &&
	     <p>
	    ...
	    <Form ref={props.innerRef} onSubmit={(e)=>testSubmit(e)}>


	    {Object.keys(cvssConfig).map((met_type, idx) => (
		<React.Fragment key={`met-${idx}`}>
		    <div className="text-center border-bottom mb-3">
			<h3 key={`metric_type_${met_type}`}>{met_type}</h3>
		    </div>
