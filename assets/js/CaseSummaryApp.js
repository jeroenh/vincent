...
const CaseSummaryApp = (props) => {
    ...
    const [activeTab, setActiveTab] = useState(searchParams.get("activeTab") || "casedetails");
				...
				{props.user.role == "coordinator" ?
				 ...
				 <>
				     {props.user.status_needed ?
				      ...
				      :
				      <span className="text-nowrap">Status {vuls.length > 0 && props.user.role == "supplier" &&  <i className="fas fa-check text-success"></i>}</span>
				     }
				 </>
                                }
			    </Nav.Link>
